...
object AndroidConversions {
  ...
  implicit def toRichRunnable(r: Runnable) = utils.RichRunnable(r)

  implicit def toRichProgressDialog(d: ProgressDialog) = utils.RichProgressDialog(d)
  implicit def toRichView(v: View) = utils.RichView(v)
  ...
  implicit def toRichListView(v: ListView) = utils.RichListView(v)
  implicit def toRichContext(c: Context) = utils.RichContext(c)
  implicit def toRichActivity(a: Activity) = new RichActivity(a)
  implicit def toRichHandler(h: Handler) = utils.RichHandler(h)
...
case class RichView(view: View) {
  import de.gyst.utils.AndroidConversions._
